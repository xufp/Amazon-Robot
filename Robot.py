...
from record_product_information.VisitRecord import get_connection
...
class Robot:
    ...
    def sign_up(self, sign_up_form, sign_up_url = r'https://www.amazon.co.uk/ap/register?openid.pape.max_auth_age=0&openid.return_to=https%3A%2F%2Fwww.amazon.co.uk%2Fgp%2Fregistry%2Fadd-item.html%2Fref%3Ddp_start-bbf_1_glance%3Fie%3DUTF8%26asin.0%3DB00UW0VG2E%26id%3D%26requestedQty.0%3D1%26sid%3D258-2478262-3616434%26sourceVendorId%3Dwebsite.wishlist.detail.add%26type%3Dwishlist&prevRID=0KAS2C9XPWFYDJ91MPFT&openid.identity=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0%2Fidentifier_select&openid.assoc_handle=gbflex&openid.mode=checkid_setup&openid.ns.pape=http%3A%2F%2Fspecs.openid.net%2Fextensions%2Fpape%2F1.0&prepopulatedLoginId=&failedSignInCount=0&openid.claimed_id=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0%2Fidentifier_select&pageId=gbflex&openid.ns=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0'):
        #sign_up_url = r'https://www.amazon.com/ap/register?_encoding=UTF8&openid.assoc_handle=usflex&openid.claimed_id=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0%2Fidentifier_select&openid.identity=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0%2Fidentifier_select&openid.mode=checkid_setup&openid.ns=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0&openid.ns.pape=http%3A%2F%2Fspecs.openid.net%2Fextensions%2Fpape%2F1.0&openid.pape.max_auth_age=0&openid.return_to=https%3A%2F%2Fwww.amazon.com%2Fgp%2Fyourstore%2Fhome%3Fie%3DUTF8%26ref_%3Dnav_custrec_newcust'):
    ...
    def sign_in(self, sign_in_url = r'https://www.amazon.co.uk/ap/signin?_encoding=UTF8&openid.assoc_handle=usflex&openid.claimed_id=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0%2Fidentifier_select&openid.identity=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0%2Fidentifier_select&openid.mode=checkid_setup&openid.ns=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0&openid.ns.pape=http%3A%2F%2Fspecs.openid.net%2Fextensions%2Fpape%2F1.0&openid.pape.max_auth_age=0&openid.return_to=https%3A%2F%2Fwww.amazon.com%2F%3Fref_%3Dnav_signin'):
        ...
        try:
            ...
            # randomly get a user from DB
            conn = get_connection()
            conn.autocommit(False)
            cursor = conn.cursor()
            SQL = 'select id, mailbox, passwd, mac from sign_user WHERE status = 0 limit 1 for update'
            cursor.execute(SQL)
            result = cursor.fetchall()
            if not result:
                print 'has no sign user from db'
            else:
                mail_box = result[0][0]
                passwd = result[0][1]
                mac = result[0][2]
                ...
                for k,v in sign_in_form.items():
                    ...
                    time.sleep(5)
                inputElement.submit()
                SQL = 'update sign_user set status = 1 where id = %d' %(int(result[0][0]))
                cursor.execute(SQL)
            conn.commit()
        except Exception, e:
    ...
    def simulate_browsing(self, brand, words, asin , possibility, qid = None):
        ...
        if qid:
            target_url = 'https://www.amazon.co.uk/%s/dp/%s/ref=sr_1_93?ie=UTF8&qid=%s&sr=8-93&keywords=%s' %(brand, asin, qid, key_words)
        else:
            target_url = 'https://www.amazon.co.uk/%s/dp/%s/ref=sr_1_93?ie=UTF8&sr=8-93&keywords=%s' %(brand, asin, key_words)
    ...
    def search_keywords(self, words):
        ...
        """
        try:
            self.driver.get(r'https://www.amazon.co.uk/')
    ...
    def store_registered_user(self, user_info):
        ...
        try:
            ...
            cursor = conn.cursor()
            SQL = 'insert into signup_user(email, password, mac) values("%s", "%s", "%s");' %(email, password, mac)
            print SQL
            cursor.execute(SQL)
            print '===========successfully add user %s to DB==============' %(user_info)
        except Exception, e:
            traceback.print_exc()
            print 'Error while adding registered user to DB\n %s'%(e.message)
            sys.exit(0)
        finally:
            cursor.close()
            conn.close()
