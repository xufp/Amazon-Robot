...
def get_proxyips():
    try:
        ...
        cursor = conn.cursor()
        SQL = 'select ip from proxy_ips WHERE status = 0;'
        cursor.execute(SQL)
        result = cursor.fetchall()
        if not result:
            print 'has no ips from proxy_ips'
            return
        return result
    finally:
        cursor.close()
...
#delete invalid ip
def delete_invalidIP(ip):
    try:
        conn = get_connection()
        conn.autocommit(False)
        cursor = conn.cursor()
        SQL = 'update proxy_ips set status = 1 WHERE ip="%s";' %(ip)
        cursor.execute(SQL)
        conn.commit()
        print 'delete ip "%s"' %(ip)
    finally:
        cursor.close()
        conn.close()
